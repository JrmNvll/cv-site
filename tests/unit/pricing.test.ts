...
describe('reservationMicroUsd', () => {
  it('estime lʼentrée à un jeton pour deux caractères, au prix dʼécriture du cache, plus max_tokens de sortie', () => {
    expect(CHARS_PER_TOKEN).toBe(2);
    expect(reservationMicroUsd(0)).toBe(MAX_TOKENS * 25);
    // Des micro-USD entiers : arrondis, jamais des fractions.
    expect(reservationMicroUsd(2)).toBe(Math.round(6.25 + MAX_TOKENS * 25));
    expect(reservationMicroUsd(3)).toBe(Math.round(12.5 + MAX_TOKENS * 25));
    // Le contexte réel (≈ 41 000 caractères) : ≈ 0,158 USD.
    expect(reservationMicroUsd(41_000)).toBe(20_500 * 6.25 + 30_000);
  });
  ...
  it('couvre le premier appel réel du 2026-09-17 : 0,128 USD facturés pour 41 000 caractères envoyés', () => {
    ...
    expect(reservationMicroUsd(41_000)).toBeGreaterThan(reel);
  });
  ...
  it('ne présume pas du cache : la réservation dépasse le coût réel dʼun appel en cache', () => {
    ...
    expect(reservationMicroUsd(chars)).toBeGreaterThan(reel);
    expect(reservationMicroUsd(-10)).toBe(MAX_TOKENS * 25);
  });
});
